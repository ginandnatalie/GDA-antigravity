...
export default function Hero({ onOpenModal, editMode }: HeroProps) {
  ...
  useEffect(() => {
    ...
    const drawParticles = () => {
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(244,162,26,${p.alpha})`;
        ctx.fill();
      });
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 90) {
            ...
            ctx.strokeStyle = `rgba(244,162,26,${0.06 * (1 - dist / 90)})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
  ...
  return (
    <section id="hero" className="min-h-[100svh] flex flex-col pt-[72px] overflow-hidden relative bg-bg">
      ...
      <div className="hero-orb absolute rounded-full pointer-events-none w-[900px] h-[900px] bg-[radial-gradient(circle,rgba(244,162,26,0.055)_0%,transparent_65%)] -top-[200px] -right-[200px]" />
      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-6 md:px-14 py-10 sm:py-12 md:py-16 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center relative z-[2] flex-1">
        <div className="hero-left">
          ...
          <h1 className="font-syne font-extrabold text-[32px] sm:text-[42px] md:text-[56px] lg:text-[76px] leading-[0.92] tracking-[-0.035em] mb-5 animate-fadeUp delay-100 relative group">
            {editMode ? (
              <textarea 
                className="w-full bg-surface/50 border border-gold/30 rounded p-2 text-text-custom outline-none focus:border-gold"
              ...
              <>
                Africa's
                <span className="block italic font-light font-dm-sans text-gold tracking-[-0.02em]">Future</span>
                <span>Technologists</span>
                <br />
                <span className="text-transparent" style={{ WebkitTextStroke: '1px var(--text-stroke)' }}>Built Here</span>
              </>
            )}
            {editMode && <span className="absolute -top-6 left-0 text-[10px] text-gold font-dm-mono uppercase">Edit Hero Title</span>}
          </h1>

          <div className="relative group mb-6 sm:mb-8">
            {editMode ? (
              <textarea 
                className="w-full bg-surface/50 border border-gold/30 rounded p-2 text-[16px] text-text-soft leading-[1.75] h-32 outline-none focus:border-gold"
            ...
            {editMode && <span className="absolute -top-6 left-0 text-[10px] text-gold font-dm-mono uppercase">Edit Hero Subtitle</span>}
          </div>
        ...
        <div className="hidden lg:flex flex-col items-center justify-center relative animate-fadeUp delay-200 min-h-[320px]">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gold-dim border border-gold/20 rounded-lg px-5 py-4 animate-float1 shadow-lg shadow-gold/5" style={{ animationDelay: '0.5s' }}>
            <div className="font-dm-mono text-[9px] tracking-[0.1em] uppercase text-gold">Active Learners</div>
        ...
        <div className="max-w-[1280px] mx-auto px-5 sm:px-6 md:px-14 py-4 sm:py-5 grid grid-cols-3 sm:grid-cols-3 md:grid-cols-5 gap-y-4 gap-x-2">
          {[
            ...
            <div key={i} className={`flex flex-col gap-0.5 text-center md:text-left px-2 md:px-4 ${i < 4 ? 'md:border-r border-border-custom' : ''}`}>
              <div className="font-syne font-extrabold text-[20px] sm:text-[24px] md:text-[28px] tracking-[-0.04em] leading-none text-text-custom">{stat.value}<span className="text-gold">{stat.suffix}</span></div>
